...
def main():
    ...
    try:
        start_year = int(input("Enter start year (default 2000): ").strip() or "2000")
    except ValueError:
        print("Invalid year input. Using 2000.")
        start_year = 2000
    ...
    print(f"Dumping CERT-FR AVI JSON from {start_year} to {current_year}")
...
"""Depuis l’année 2000, les publications du CERT suivent un format d’identifiant structuré 
composé d’un préfixe, de l’année, du type de publication et d’un compteur numérique. Entre 2000 et 2013, 
les avis étaient généralement référencés sous la forme CERTA-AAAA-AVI-NNN, 
avec un numéro sur 3 chiffres (exemple : CERTA-2010-AVI-490). À partir de 2014, 
le CERT-FR a fait évoluer ce format en remplaçant le préfixe CERTA par CERTFR, 
tout en conservant la même structure (année + type + compteur). 
De 2014 à 2022, la numérotation des avis (AVI) est restée le plus souvent sur 3 chiffres (exemple : CERTFR-2019-AVI-067). 
Enfin, depuis 2023, le CERT-FR utilise une numérotation sur 4 chiffres pour les identifiants de type AVI (exemple : CERTFR-2023-AVI-0067), 
afin d’augmenter la capacité de numérotation tout en gardant un format cohérent et standardisé."""